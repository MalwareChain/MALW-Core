...
#ifndef MALWARECHAIN_ZMALWWALLET_H
#define MALWARECHAIN_ZMALWWALLET_H
...
public:
    CzMALWWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMALW(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMALW(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MALWARECHAIN_ZMALWWALLET_H
