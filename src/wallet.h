...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MALW at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMALW send
enum ZerocoinSpendStatus {
    ZMALW_SPEND_OKAY = 0,                            // No error
    ZMALW_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMALW_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMALW_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMALW_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMALW_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMALW_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMALW_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMALW_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMALW_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMALW_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMALW_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMALW_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMALW_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMALW_SPENT_USED_ZMALW = 14,                      // Coin has already been spend
    ZMALW_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMALW_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZMalwBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMALWOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmalwAuto) const;
    ...
    */
    ...
    CzMALWWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMALWTracker> zmalwTracker;
    ...
    void setZWallet(CzMALWWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmalwTracker = std::unique_ptr<CzMALWTracker>(new CzMALWTracker(strWalletFile));
    }

    CzMALWWallet* getZWallet() { return zwalletMain; }
    ...
    void setZMalwAutoBackups(bool fEnabled)
    {
