...
CzMALWTracker::CzMALWTracker(std::string strWalletFile)
{
...
CzMALWTracker::~CzMALWTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMALWTracker::Init()
{
...
bool CzMALWTracker::Archive(CMintMeta& meta)
{
...
bool CzMALWTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMALWTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMALWTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMALWTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMALWTracker::GetSerialHashes()
{
...
CAmount CzMALWTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMALWTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMALWTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMALWTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMALWTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMALWTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMALWTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMALWTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMALWTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMALWTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMALWTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMALWTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMALWTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMALWTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMALWTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMALWTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMALWTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmalw from DB\n", __func__, listDeterministicDB.size());
    }
