...
#include "config/malwarechain-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("malwarechain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "malwarechain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "malwarechaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MALW and zMALW money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 21103, 22103));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MALW/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MALW/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, malwarechain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MALW/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MalwareChain specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:21103"));
    ...
    strUsage += HelpMessageOpt("-backupzmalw=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMalw minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmalwbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMALW backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizemalwarechainamount=<n>     " + strprintf(_("Keep N MALW anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 21104, 22104));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i MalwareChain ltd"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("malwarechain-loadblk");
...
/** Sanity checks
 *  Ensure that MalwareChain is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MalwareChain Core is shutting down."));
...
#endif
    // Make sure only a single MalwareChain process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MalwareChain Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MalwareChain version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MalwareChain specific: zerocoin and spork DB's
                ...
                // MalwareChain: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMALWMinted();
                        RecalculateZMALWSpent();
                    }
                    RecalculateMALWSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MalwareChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MalwareChain Core to complete") << "\n";
        ...
        zwalletMain = new CzMALWWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zMALW wallet..."));

        bool fEnableZMalwBackups = GetBoolArg("-backupzmalw", true);
        pwalletMain->setZMalwAutoBackups(fEnableZMalwBackups);
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeMalwareChainAmount = 2;
...
//    }
//
//    nAnonymizeMalwareChainAmount = GetArg("-anonymizemalwarechainamount", 0);
//    if (nAnonymizeMalwareChainAmount > 999999) nAnonymizeMalwareChainAmount = 999999;
//    if (nAnonymizeMalwareChainAmount < 2) nAnonymizeMalwareChainAmount = 2;
    ...
    LogPrintf("Anonymize MalwareChain Amount %d\n", nAnonymizeMalwareChainAmount);
    ...
    /* Denominations
       ...
       For example:
       1MALW+1000 == (.1MALW+100)*10
       10MALW+10000 == (1MALW+1000)*10
    */
