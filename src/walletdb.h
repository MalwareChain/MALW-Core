...
public:
    ...
    bool WriteZMALWSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMALWSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMALWSeed_deprecated(uint256& seed);
    bool EraseZMALWSeed();
    bool EraseZMALWSeed_deprecated();

    bool WriteZMALWCount(const uint32_t& nCount);
    bool ReadZMALWCount(uint32_t& nCount);
