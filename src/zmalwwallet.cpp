...
CzMALWWallet::CzMALWWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmalw seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMALWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMALWSeed_deprecated()) {
                    LogPrintf("%s: Updated zMALW seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmalw seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmalw wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMALWWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMALWCount(nCountLastUsed);
    else if (!walletdb.ReadZMALWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMALWWallet::Lock()
{
    seedMaster = 0;
}

void CzMALWWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMALWWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMALW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMALWWallet::LoadMintPoolFromDB()
{
...
void CzMALWWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMALWWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMALWWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmalwTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMALWWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMALW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmalwTracker which also adds to database
    pwalletMain->zmalwTracker->Add(dMint, true);
...
void CzMALWWallet::SeedToZMALW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMALWWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMALWWallet::GenerateDeterministicZMALW(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMALWWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMALWWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
