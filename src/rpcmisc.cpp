...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total malwarechain balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zMALWsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zMALW denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zMALW denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zMALW denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zMALW denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zMALW denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zMALW denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zMALW denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zMALW denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zMALW denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in malwarechain/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in malwarechain/kb\n"
    ...
    UniValue zmalwObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zmalwObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zmalwObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zMALWsupply", zmalwObj));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are malwarechain addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) malwarechain address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"malwarechainaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"malwarechainaddress\"  (string, required) The malwarechain address to use for the signature.\n"
