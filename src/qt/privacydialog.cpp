                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zMALWpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zMALW ought to be enough for anybody." - Bill Gates, 2017
    ui->zMALWpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMALWSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMALWpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMALW_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMALW is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMALW...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMALW in ") +
...
void PrivacyDialog::on_pushButtonSpendzMALW_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMALW is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMALW
        sendzMALW();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMALW
    sendzMALW();
}

void PrivacyDialog::on_pushButtonZMalwControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZMalwControlDialog* zMalwControl = new ZMalwControlDialog(this);
    zMalwControl->setModel(walletModel);
    zMalwControl->exec();
}

void PrivacyDialog::setZMalwControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMALWSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMALW()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MalwareChain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMALWpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMALW is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MALW </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMALW</b>";
    ...
    // use mints from zMALW selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMalwControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMalwControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMALW require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zMALW"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZMALW_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMALW require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zMALW"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zMALW was spent successfully update the addressbook with the label
    ...
    // Clear zmalw selector in case it was used
    ZMalwControlDialog::setSelectedMints.clear();
    ui->labelzMALWSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMALW Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Malw, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMALW Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zmalwTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMALW </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMALW "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMALW "));
    ui->labelzMALWAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMALWSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzMALW->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zMALW
        ui->pushButtonMintzMALW->setEnabled(false);
        ui->pushButtonMintzMALW->setToolTip(tr("zMALW is currently disabled due to maintenance."));

        // Spend zMALW
        ui->pushButtonSpendzMALW->setEnabled(false);
        ui->pushButtonSpendzMALW->setToolTip(tr("zMALW is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zMALW
        ui->pushButtonMintzMALW->setEnabled(true);
        ui->pushButtonMintzMALW->setToolTip(tr("PrivacyDialog", "Enter an amount of MALW to convert to zMALW", 0));

        // Spend zMALW
        ui->pushButtonSpendzMALW->setEnabled(true);
        ui->pushButtonSpendzMALW->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}