...
#include "ui_zmalwcontroldialog.h"
...
std::set<std::string> ZMalwControlDialog::setSelectedMints;
std::set<CMintMeta> ZMalwControlDialog::setMints;

ZMalwControlDialog::ZMalwControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMalwControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMalwControlDialog::~ZMalwControlDialog()
{
    delete ui;
}

void ZMalwControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMalwControlDialog::updateList()
{
...
void ZMalwControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMalwControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMalw_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMalwControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMalwControlDialog::GetSelectedMints()
{
...
void ZMalwControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
