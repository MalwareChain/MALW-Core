...
// Copyright (c) 2017-2019 MalwareChain LLC
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MALW)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sMALWPercentage, QString& szMALWPercentage)
{
    int nPrecision = 2;
    ...
    szMALWPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sMALWPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount malwAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = malwAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // MALW labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, malwAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelMALWPercent->setText(sPercentage);
    ui->labelzMALWPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zMALW.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in malwarechain.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in malwarechain.conf");
    }
    ...
    bool showMALWAvailable = settingShowAllBalances || malwAvailableBalance != nTotalBalance;
    bool showWatchOnlyMALWAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showMALWPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyMALWPending = watchUnconfBalance != 0;
    bool showMALWLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyMALWLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showMALWAvailable || showWatchOnlyMALWAvailable);
    ui->labelBalanceText->setVisible(showMALWAvailable || showWatchOnlyMALWAvailable);
    ui->labelWatchAvailable->setVisible(showMALWAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showMALWPending || showWatchOnlyMALWPending);
    ui->labelPendingText->setVisible(showMALWPending || showWatchOnlyMALWPending);
    ui->labelWatchPending->setVisible(showMALWPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showMALWLocked || showWatchOnlyMALWLocked);
    ui->labelLockedBalanceText->setVisible(showMALWLocked || showWatchOnlyMALWLocked);
    ui->labelWatchLocked->setVisible(showMALWLocked && showWatchOnly);
    ...
    bool showzMALWAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzMALWUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzMALWImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzMALWAvailable);
    ui->labelzBalanceMatureText->setVisible(showzMALWAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzMALWUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzMALWUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzMALWImmature);
    ui->labelzBalanceImmatureText->setVisible(showzMALWImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelMALWPercent->setVisible(showPercentages);
    ui->labelzMALWPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MALW")
    updateDisplayUnit();
}
