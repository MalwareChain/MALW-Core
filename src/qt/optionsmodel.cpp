...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MALW);
    ...
    if (!settings.contains("nAnonymizeMalwareChainAmount"))
        settings.setValue("nAnonymizeMalwareChainAmount", 1000);

    nAnonymizeMalwareChainAmount = settings.value("nAnonymizeMalwareChainAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMalwareChainAmount"))
        SoftSetArg("-anonymizemalwarechainamount", settings.value("nAnonymizeMalwareChainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in malwarechain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMalwareChainAmount:
            return QVariant(nAnonymizeMalwareChainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMalwareChainAmount:
            nAnonymizeMalwareChainAmount = value.toInt();
            settings.setValue("nAnonymizeMalwareChainAmount", nAnonymizeMalwareChainAmount);
            emit anonymizeMalwareChainAmountChanged(nAnonymizeMalwareChainAmount);
            break;
        case CoinControlFeatures:
