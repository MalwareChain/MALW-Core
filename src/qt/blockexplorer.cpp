...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::MALW, nValue);
...
{
    std::string genesisblockhash = "000007cabcfba44b7a728f1746bdec842c9fefab32e8e9df3e18d28ecc265eb3";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (malwarechain.conf).");
            QMessageBox::warning(this, "MalwareChain Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
