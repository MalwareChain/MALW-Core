...
====================================
This guide will show you how to build malwarechaind (headless client) for OSX.
...
### Building `malwarechaind`

1. Clone the github tree to get the source code and go into the directory.

        git clone https://github.com/MalwareChainOSS/MalwareChain.git
        cd MalwareChain
...
5.  (Optional) You can also install malwarechaind to your path:
...
4. Enter "malwarechain-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `malwarechaind` for your own use.

malwarechaind/malwarechain-cli binaries are not included in the malwarechain-Qt.app bundle.

If you are building `malwarechaind` or `malwarechain-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the MalwareChain-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./malwarechaind`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./malwarechaind` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=malwarechainrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/MalwareChain/malwarechain.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/MalwareChain/malwarechain.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/MalwareChain/debug.log

Other commands:
-------

    ./malwarechaind -daemon # to start the malwarechain daemon.
    ./malwarechain-cli --help  # for a list of command-line options.
    ./malwarechain-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure MalwareChain with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the MalwareChain wallet:
...
You have the choice to build the GUI MalwareChain wallet as a Mac OSX app, described in “How to build the MalwareChain-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the MalwareChain-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find MalwareChain-Qt.dmg inside your MalwareChain folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/MalwareChain/src/qt/malwarechain-qt