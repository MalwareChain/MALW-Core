...
VERSION=
commit=false
url=https://github.com/MalwareChainOSS/MALW-Core
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the malwarechain, gitian-builder, gitian.sigs, and malwarechain-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/MalwareChainOSS/MALW-Core
...
then
	# Make output folder
	mkdir -p "./malwarechain-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit malwarechain=${COMMIT} --url malwarechain=${url} ../malwarechain/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/malwarechain-*.tar.gz build/out/src/malwarechain-*.tar.gz ../malwarechain-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit malwarechain=${COMMIT} --url malwarechain=${url} ../malwarechain/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/malwarechain-*.zip build/out/malwarechain-*.exe ../malwarechain-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit malwarechain=${COMMIT} --url malwarechain=${url} ../malwarechain/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/malwarechain-*.tar.gz build/out/malwarechain-*.dmg ../malwarechain-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit malwarechain=${COMMIT} --url malwarechain=${url} ../malwarechain/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../malwarechain/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/malwarechain-*.tar.gz build/out/src/malwarechain-*.tar.gz ../malwarechain-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../malwarechain/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/malwarechain-*win64-setup.exe ../malwarechain-binaries/${VERSION}
	    mv build/out/malwarechain-*win32-setup.exe ../malwarechain-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../malwarechain/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/malwarechain-osx-signed.dmg ../malwarechain-binaries/${VERSION}/malwarechain-${VERSION}-osx.dmg
	fi
	popd || exit
